...
import { validarVolantesEditar } from "@/schemas/productos/volantes";
...
export const GET = async (_: NextRequest, { params }: any) => {
  try {
    ...
    const orden = await VolantesModel.obtener(idValido);
...
export const PATCH = async (req: NextRequest, { params }: any) => {
  try {
    ...
    const productoValidado = validarVolantesEditar(body);

    const actualizado = await VolantesModel.actualizar(idValido, productoValidado);

    if (!actualizado) return orden404Response();

    const ordenActualizada = await VolantesModel.obtener(id);
...
export const DELETE = async (_: NextRequest, { params }: any) => {
  try {
    ...
    const eliminado = await VolantesModel.eliminar(idValido);
